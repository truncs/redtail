...
void Image2RTSPNodelet::onInit() {
    ...
    string pipeline_tail =  " ! video/x-h264, profile=baseline ! rtph264pay name=pay0 pt=96 )"; // Gets completed based on rosparams below
    ...
    for(XmlRpc::XmlRpcValue::ValueStruct::const_iterator it = streams.begin(); it != streams.end(); ++it)
    {
        ...
        else if (stream["type"]=="topic")
        {
            ...
            * so we know to stop subscribing when no-one is connected. */
            ...
            pipeline = "( appsrc name=imagesrc do-timestamp=true min-latency=0 max-latency=0 max-bytes=1000 is-live=true ! videoconvert ! videoscale ! " + caps + " ! omxh264enc insert-sps-pps=1 bitrate=" + bitrate + pipeline_tail;
