...
if [[ -z "${TENSORRT_TAR_FILE}" ]]; then
    ...
    echo "Example: build_redtail_image.sh /data/downloads/NVIDIA/TensorRT-4.0.1.6.Ubuntu-16.04.4.x86_64-gnu.cuda-9.2.cudnn7.1"
    exit 1
fi
...
# Build the image.
docker build -t redtail-ardupilot-sim:kinetic${IMAGE_TAG_SUFFIX} --build-arg TENSORRT_TAR_FILE=`basename ${TENSORRT_TAR_FILE}` -f Ardupilot.kinetic .
