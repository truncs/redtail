...
if [[ ! -L "${CATKIN_WS}/src/caffe_ros" ]]; then
    ...
    catkin build caffe_ros_node -DCUDA_USE_STATIC_CUDA_RUNTIME=OFF
fi
...
if [[ ! -L "${CATKIN_WS}/src/redtail_debug" ]]; then
    ln -s ${REDTAIL_PATH}/ros/packages/redtail_debug ${CATKIN_WS}/src/
fi
