...
picture_camera() {
	if ! ps -ef | grep [i]mage:=/zed/zed_node/left/image_rect_color 
	then
...
picture_depth() {
        if ! ps -ef | grep [c]onfidence/confidence_image
        then
           rosrun image_view image_saver image:=/zed/zed_node/confidence/confidence_image _save_all_image:=false _filename_format:=/home/apsync/dflogger/dataflash/zed_image/zed_depthgray%04i.jpg __name:=zed_depth_image_saver > /dev/null &
           sleep 6
        fi
